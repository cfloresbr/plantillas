...
(function ($, Drupal) {
  ...
  let changeBlockLanguage = function() {
    let options, select, active;
    options = "";
    active = "";

    if ($(".select-language-processed").length > 0) {
      return;
    }

    $('#block-languageswitcher a.language-link').each(function () {
      options += '<option value="' + $(this).attr("href") + '">' + $(this).text() + '</option>';
    });

    select = "";
    select += '<div class="form-item__dropdown select-language-processed">';
    select += '  <select id="language" class="form-item__select form-item--language" name="language" placeholder="Idioma" aria-label="Lenguaje">';
    select += options;
    select +=  '</select>';
    select +=  '  </div>';

    $('#block-languageswitcher').html(select);
    $('#block-languageswitcher #language').change(function() {
  ...
  Drupal.behaviors.languageswitcher = {
    attach: function (context) {
      ...
      // setTimeout(function(){changeBlockLanguage()}, 100);

      $('#block-languagedropdownswitcher .lang-dropdown-select-element option').each(function () {
...
(function ($, Drupal) {
  ...
  // Set click on search in general site serach in header.
  function searchTextByClick(context) {
    let form = $('#block-snpc-searching-block form', context);
    form.parent().append('<a id="lupa" href="#"><i class="fa fa-search"></i></a>');
    // Add click on icon.
    document.getElementById('lupa').addEventListener("click", function() {
      $(this).attr('href', '/buscar?texto=' + form.find('input').val());
    });
  }

  $(document).ready(function() {
    //Ajustar flechas al medio - destacado grande
    let $window = $(window);
    $window.on('resize', function() {
      let $ancho_pantalla = $( window ).width();
      if ($ancho_pantalla > 1140) {
        $ancho_pantalla = 1140;
      }
      $ancho_pantalla = $ancho_pantalla - 40;
      let $alto_pantalla = $ancho_pantalla*6/16;

      let $media_slick = $('.media--slick.media--image');
      if ($media_slick.parent('.slide__media').length) {
        $media_slick.css('height', $alto_pantalla);
        $media_slick.css('max-height', $alto_pantalla);
      }

      $('.slick--skin--fullwidth nav.slick__arrow').css('top', $alto_pantalla/2);

      //altura arrows destacado
      let $altura_destacado = $('.destacado-slideshow-wrapper .slick-active .destacado__image').height();
      $('.destacado-slideshow-wrapper nav.slick__arrow').css('top', $altura_destacado/2);
    });
    setTimeout(function() {
    ...
    attach: function (context) {
      if (navigator.language === 'es-US') {
        document.addEventListener("webviewerloaded", function() {
          window[0].PDFViewerApplicationOptions.set("locale", "es-ES");
          let iframe_pdf = $('iframe.pdf').contents();
          iframe_pdf.find('#presentationMode').hide();
          iframe_pdf.find('#openFile').hide();
          iframe_pdf.find('#print').hide();
          iframe_pdf.find('#viewBookmark').hide();
        });
      }
      // Se elimina caption si está vacio.
      let $slickLightBoxElements = $('.blazy--slick-lightbox', context);
      if ($slickLightBoxElements.length) {
        $slickLightBoxElements.on('show.slickLightbox', function() {
          $('.slick-lightbox .slick-lightbox-slick-caption').each(function() {
            let caption = $(this);
            if (!caption.text()) {
              caption.css('background', 0);
            }
            else {
              let el = caption;
              let divHeight = el.prop('scrollHeight') - 30;
              let lineHeight = parseInt(el.css('line-height'));
              let lines = divHeight / lineHeight;
              if (lines > 2) {
                caption.css('overflow', 'hidden');
                caption.after('<p class="read-more">' +
                  '<a href="#" class="link link-read-more">Lee más</a>' +
                  '<a href="#" class="link link-read-less">Lee menos</a>' +
                  '</p>');
                let $read_more = caption.parent().find('.read-more');
                let $p_leer_mas = $('.link.link-read-more', $read_more);
                let $p_leer_menos = $('.link.link-read-less', $read_more);
                $p_leer_menos.hide();
                $p_leer_mas.on('click', function() {
                  caption.css({
                    'position': 'absolute',
                ...
                $p_leer_menos.on('click', function() {
                  caption.scrollTop(0);
                  caption.css({
                    'position': 'relative',
                    'height': '80px',
      ...
      //Set click on search in general site serach in header.
      searchTextByClick(context);

      //Poner componentes como bloques
      let $content = $('#block-snpc-content', context).once('snpc-content');
      let $paragraphs = $('.field--name-field-paragraph', $content);

      let $bloquepiloto = $('#block-views-block-institucion-menu-pages', context);
      if (!$bloquepiloto[0]){
        $bloquepiloto = $('#block-snpc-content', context);
      }
      $($paragraphs.children().get().reverse()).each(function () {
        $(this).addClass(['block', 'paragraph-component']);
        $bloquepiloto.after($(this));
      });
      $paragraphs.remove();

      //detectar si destacado slide tiene solo un elemento
      //margin-left: calc(-50vw + 50%);
      //margin-right: calc(-50vw + 50%);
      let $ds = $('.destacado-slideshow', context);
      if($ds.length) {
        $ds.each(function () {
          if ($(this).find('.slick--optionset--destacado-grande > .slick__slide').length){
            $(this).css('margin-left', 'calc(-50vw + 50%)');
            $(this).css('margin-right', 'calc(-50vw + 50%)');
          }
        });
      }
      let $dsm = $('.destacado-slideshow-minisitio', context);
      if($dsm.length) {
        $dsm.each(function () {
          if ($(this).find('.slick--optionset--destacado-grande > .slick__slide').length){
            // $(this).css('margin-left', 'calc(-50vw + 50%)');
      ...
      //Detectar solo una noticia destacada
      let $nd = $('.noticias-destacadas', context);
      if ($nd.length) {
        $nd.each(function () {
          if ($(this).find('.field--name-field-noticias > div').length === 1){
            $(this).addClass('one-element');
          }
        });

      }
      let $ndm = $('.noticias-minisitios', context);
      if ($ndm.length) {
        $ndm.each(function () {
          if ($(this).find('.field--name-field-noticias > div').length === 1){
            $(this).addClass('one-element');
      ...
      //Incluir bloques dentro de landing si este posee componentes
      let $main = $('.main-content > div', context).once('loaded-main');
      let $bloques = $main.find('> div.block:not(#block-snpc-content):not(#block-snpc-breadcrumbs):not(#block-snpc-page-title)', context);
      let $landing = $main.find('article.node--landing');
      if ($landing.length) {
        $landing.find('.field--name-field-componentes').append($bloques);
      }

      //dejar field recursos-block como bloque
      let $recursos = $('#recursos-block', context).once('recursos-field');
      if ($recursos.length) {
        $main.append($recursos);
      }

      // padding-bottom destacado-slideshow
      $('.destacado-slideshow-minisitio', context).parent().addClass('destacado-slideshow-wrapper');
      $('.destacado-slideshow', context).parent().addClass('destacado-slideshow-wrapper');
      let  component_landing = $('.snpc-node-type-landing .field--name-field-componentes');
      if (component_landing.length) {
        let second_child = component_landing.children().eq(1);
        if (second_child.hasClass('destacado-slideshow-wrapper')) {
          $('.destacado-slideshow', second_child).attr('style', 'max-width: 1140px !important');
...
(function ($, Drupal) {
  ...
  Drupal.behaviors.snpc_ui_admin = {
    attach: function (context, settings) {
      // Alter moderation state select for nodes form.
      $('label', context).once('admin_ui').each(function() {
        // Get current node form (context).
        var label = $(this);
        var replaced = label.html().replace(/Name/g, 'Nombre');
        label.html(replaced);
      });

      // Agrega palabras claves con valor de campo Titulo.
      if($('#edit-field-yoast-seo-0-yoast-seo-focus-keyword').length){
        $('#edit-title-0-value').focusout(function() {
          var title_node = $(this).val();
          var keyword_yoast = $('#edit-field-yoast-seo-0-yoast-seo-focus-keyword').val();

          if (keyword_yoast === '') {
            $('#edit-field-yoast-seo-0-yoast-seo-focus-keyword').val(title_node);
          }
          else {
            // No cambia texto de keyword yoast
          }
